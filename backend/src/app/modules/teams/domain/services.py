...
class TeamService:
    ...
    # ── Equipos ──────────────────────────────────────────────────────────────
    async def create_team(self, data: CreateTeamRequest) -> TeamResponse:
        if await self.repo.get_team_by_name(data.name) is not None:
            raise ConflictError("Ya existe un equipo con ese nombre")
    ...
    async def update_team(self, team_id: UUID, data: UpdateTeamRequest) -> TeamResponse:
        team = await self._get_active_team(team_id)
        if data.name and data.name != team.name:
            existing = await self.repo.get_team_by_name(data.name)
    ...
    async def add_member(
        ...
        await self._get_active_team(team_id)
    ...
    # ── Helpers ──────────────────────────────────────────────────────────────
    async def _get_active_team(self, team_id: UUID) -> Team:
